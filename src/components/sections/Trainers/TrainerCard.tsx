      ...
      >
        <div className="relative h-[26rem] overflow-hidden">
          ...
          <div className="absolute inset-x-0 bottom-0 p-6">
            <p className="text-sm uppercase tracking-[0.24em] text-(--accent-ink)">{trainer.specialty}</p>
            <h3 className="mt-3 display-heading text-3xl text-(--foreground)">{trainer.name}</h3>
          </div>
          <div className="absolute right-6 top-6 flex translate-y-2 gap-2 opacity-0 transition duration-300 group-hover:translate-y-0 group-hover:opacity-100">
            {trainer.socials.map((social) => {
              ...
              return (
                <a
                  key={social.label}
                  className="focus-ring inline-flex h-10 w-10 items-center justify-center rounded-full border border-(--border) bg-(--surface-overlay) text-(--foreground) backdrop-blur-sm"
                  href={social.href}
        ...
        <div className="p-6">
          <p className="text-sm leading-7 text-(--muted)">{trainer.bio}</p>
        </div>
      </motion.article>
    </motion.div>
  )
